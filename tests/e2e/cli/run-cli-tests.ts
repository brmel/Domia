...
async function runTests() {
    ...
    if (!platform || platform === 'web' || platform === 'all') {
        const webSmokePassed = await runTest('tests/e2e/cli/web-test.ts', 'Web Platform Smoke');
        results.push({ name: 'Web Smoke', passed: webSmokePassed });

        const webFeaturesPassed = await runTest('tests/e2e/cli/web-features-test.ts', 'Web Platform Feature Suite');
        results.push({ name: 'Web Features', passed: webFeaturesPassed });
    }
    
    if (platform === 'electron' || platform === 'all') {
        const electronPassed = await runTest('tests/e2e/cli/electron-test.ts', 'Electron Platform');
        results.push({ name: 'Electron', passed: electronPassed });
    }

    if (platform === 'counter' || platform === 'all') {
        const counterPassed = await runTest('tests/e2e/cli/counter-scenario-test.ts', 'Counter Scenarios (Fast + Slow)');
        results.push({ name: 'Counter Scenarios', passed: counterPassed });
    }

    if (platform === 'recording' || platform === 'all') {
        const recordingPassed = await runTest('tests/e2e/cli/recording-scenario-test.ts', 'Recording Tools Scenario');
        results.push({ name: 'Recording Tools', passed: recordingPassed });
    }

    if (platform === 'shell' || platform === 'all') {
        const shellPassed = await runTest('tests/e2e/cli/shell-capability-test.ts', 'Shell Capability Gate');
