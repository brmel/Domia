...
async function runFeatureCases(baseUrl: string): Promise<FeatureCaseResult[]> {
    ...
    const temporalResult = await runCLITest({
        url: baseUrl,
        prompt: 'make sure button Start Scenario is present',
        maxSteps: 3,
        headless: true,
        ...
        temporalBurstIntervalMs: 80,
        temporalMaxFramesPerWindow: 8,
        temporalPromptTokenBudget: 350,
        temporalPersistWindow: true,
        temporalRedactSensitive: true
    });
    logTestResult('Web Feature: Temporal Observation', temporalResult);

    let temporalArtifactsOk = false;
    let temporalArtifactError: string | undefined;
    if (temporalResult.success && temporalResult.runId) {
        const timelinePath = await findRunStepAsset(temporalResult.runId, '_timeline.json');
        if (!timelinePath) {
            temporalArtifactError = 'No timeline artifact emitted for temporal observation run.';
        } else {
            ...
            const parsed = JSON.parse(timelineContent) as {
                ...
                frames?: Array<{ timestamp?: number; intervalMs?: number; note?: string }>;
                fromTimestamp?: number;
                toTimestamp?: number;
            };
            const timelineAssets = await listRunStepAssets(temporalResult.runId, '_timeline.json');
            const frameCount = Array.isArray(parsed.frames) ? parsed.frames.length : 0;
            const hasManyFrames = frameCount >= 3;
            const hasExpectedMode = parsed.mode === 'forensic';
            const hasSingleTimelineAsset = timelineAssets.length === 1;
            const hasPositiveSpan = typeof parsed.fromTimestamp === 'number'
                && typeof parsed.toTimestamp === 'number'
                && parsed.toTimestamp > parsed.fromTimestamp;
            const hasPositiveIntervals = Array.isArray(parsed.frames)
                && parsed.frames.every((frame) => typeof frame?.intervalMs === 'number' && frame.intervalMs > 0);
            const isChronological = Array.isArray(parsed.frames)
                && parsed.frames.every((frame, index, frames) => {
            ...
            const hasExpectedNotes = Array.isArray(parsed.frames)
                && parsed.frames.every((frame) => frame?.note === 'perception-capture');
            const selectedCountMatches = typeof parsed.selectedFrameCount === 'number' && parsed.selectedFrameCount === frameCount;
            const droppedCountValid = typeof parsed.droppedFrameCount === 'number' && parsed.droppedFrameCount >= 0;

            temporalArtifactsOk = hasManyFrames
                && hasExpectedMode
                && hasSingleTimelineAsset
                && hasPositiveSpan
                && hasPositiveIntervals
                && isChronological
                && hasExpectedNotes
                && selectedCountMatches
                && droppedCountValid;
            if (!temporalArtifactsOk) {
                temporalArtifactError = `Timeline artifact did not prove multi-snapshot capture (frames=${frameCount}).`;
            }
        }
    } else {
        temporalArtifactError = 'Temporal run itself failed or did not expose runId.';
    }

    results.push({
        name: 'temporal-window-persistence',
        passed: temporalArtifactsOk,
    ...
    logTestResult('Web Feature: Temporal Disabled Gating', temporalOffResult);

    let temporalOffOk = false;
    let temporalOffError: string | undefined;
    if (temporalOffResult.success && temporalOffResult.runId) {
        const timelineAssets = await listRunStepAssets(temporalOffResult.runId, '_timeline.json');
        temporalOffOk = timelineAssets.length === 0;
        if (!temporalOffOk) {
            temporalOffError = 'Temporal artifacts were emitted even though temporal observation was disabled.';
        }
    } else {
        temporalOffError = 'Temporal disabled gating run failed before validation.';
    }

    results.push({
        name: 'temporal-disabled-gating',
        passed: temporalOffOk,
        ...(temporalOffOk ? {} : { details: temporalOffError || 'Temporal disabled gating validation failed.' })
    });
