...
import { container, registerCoreServices } from '../composition-root';
...
program
    ...
    .action(async (options) => {
        ...
        let { url, prompt, steps, headless } = options;
        ...
        try {
            ...
            const input = {
                ...
                options: {
                    ...
                    headless: !!headless, // Commander --no-headless sets headless=false
                },
            };
            ...
            for await (const event of generator) {
                switch (event.type) {
                    case 'started':
                        // console.log(chalk.blue('Agent started'));
                        break;
                    case 'step_complete':
                        // Handled by ConsoleViewHost, but we could add spinner updates here
                        break;
                    case 'completed':
                        if (event.success) {
